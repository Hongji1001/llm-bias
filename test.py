...
data_list = ["adult", "acs", "mdgender", "wikitalk"]
# "wikibias"
# data_list = ["age", "racial", "appearance", "bodyshaming", "lgbt", "socioeconomic"]
...
def main():
    ...
    for model_ in args.model_name_or_path:
        for data in data_list:
            ...
            model = AutoModelForSequenceClassification.from_pretrained(model_, num_labels=num_labels)
            train_mlm_for_classification(model, tokenizer, train_dataset, f"checkpoint/{model_.replace('/', '-')}_{data}_normal")
    ...
    for model_ in args.model_name_or_path:
        for data in data_list:
            ...
            num_labels = train_dataset['label'].nunique()
            model = AutoModelForSequenceClassification.from_pretrained(
                get_newest_folder(f"checkpoint/{model_.replace('/', '-')}_{data}_normal"),
                num_labels=num_labels)
            output_file_path = Path(__file__).resolve().parent / 'outputs'
            output_file_path.mkdir(parents=True, exist_ok=True)
            output_file_path = output_file_path / 'metrics'
            output_file_path.mkdir(parents=True, exist_ok=True)
            filename = output_file_path / f"eval_output_{data}_{model_.replace('/', '-')}.log"
            test_mlm_for_classification(model, tokenizer, train_dataset,
                                        filename)
