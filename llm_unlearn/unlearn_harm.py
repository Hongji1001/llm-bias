...
from utils import (
    ...
    create_bias_benchmark_from_dataset,
...
def main(args) -> None:
    ...
    (
        ...
        train_normal_loader,
        lr_scheduler,
    ) = accelerator.prepare(model, optimizer, train_bad_loader,
                            train_normal_loader, lr_scheduler)

    model.train()

    # Reference model for computing KL.
    pretrained_model = AutoModelForCausalLM.from_pretrained(args.model_name, torch_dtype=torch.float32)
    ...
    while bad_loss < args.max_bad_loss and idx < args.max_unlearn_steps:
        for bad_batch, normal_batch in zip(train_bad_loader,
                                           train_normal_loader):
            ############ GA on answer only. ############
            ...
            normal_loss = compute_kl(pretrained_model, model, normal_batch,
                                     device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bad_weight",
                        type=float,
                        default=0.5,
