...
class XLNetModel:
    def __new__(cls, model_name_or_path):
        return transformers.XLNetModel.from_pretrained(model_name_or_path)
...
class OPT1_3Model:
    def __new__(cls, model_name_or_path):
        return transformers.OPTModel.from_pretrained(model_name_or_path)

class CTRLModel:
    def __new__(cls, model_name_or_path):
        return transformers.CTRLModel.from_pretrained(model_name_or_path)
...
class INLPXLNetModel(_INLPModel):
    def __new__(cls, model_name_or_path, projection_matrix):
        super().__init__(cls, model_name_or_path, projection_matrix)
        model = transformers.XLNetModel.from_pretrained(model_name_or_path)
        model.register_forward_hook(cls.func)
        return model

class INLPLlama2Model(_INLPModel):
    def __new__(cls, model_name_or_path, projection_matrix):
        super().__init__(cls, model_name_or_path, projection_matrix)
...
class INLPOpt1_3Model(_INLPModel):
    def __new__(cls, model_name_or_path, projection_matrix):
        super().__init__(cls, model_name_or_path, projection_matrix)
        model = transformers.OPTModel.from_pretrained(model_name_or_path)
        model.register_forward_hook(cls.func)
        logging.info("Using INLPOpt1_3Model"        return model

class INLPCTRLModel(_INLPModel):
    def __new__(cls, model_name_or_path, projection_matrix):
        super().__init__(cls, model_name_or_path, projection_matrix)
        model = transformers.CTRLModel.from_pretrained(model_name_or_path)
        model.register_forward_hook(cls.func)
        return model
