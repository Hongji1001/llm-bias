...
def create_bias_benchmark_from_dataset(tokenizer, dataset, fraction=1.0, batch_size=4):
    """
    ...
    def preproccess(examples):
        ...
        for i in range(len(examples["prompts"])):
            # Subsample if needed.
            if random.random() > fraction:
                continue

            prompt = examples["prompts"][i]
            response_list = []

            response_list.append(examples["texts"][i])
